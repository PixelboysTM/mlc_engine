...
fn parse_channels(channels_raw: Option<&Value>) -> Option<HashMap<String, FixtureChannel>> {
    ...
    if let Some(avail) = channels_raw.and_then(|v| v.as_object().log(|| println!("'availableChannels' must be an object"))) {
        for (k, v) in avail {
...
fn parse_dmx_range(dmx_range: Option<&Value>, value_resolution: ValueResolution) -> Option<DmxRange> {
    let range = dmx_range?.as_array()?.iter().filter_map(|e| e.as_u64()).collect::<Vec<_>>();
    if range.len() != 2 {
    ...
    match t {
        ...
        "Intensity" => {
            if let Some(i) = Unit::<Brightness>::parse(raw_cap, false) {
                ...
                Some(FixtureCapability::Intensity(Intensity { brightness_start: Br::Percentage(Percentage::new(0.0)), brightness_end: Br::Percentage(Percentage::new(1.0)) }))
            }
        }
        "ColorIntensity" => {
        ...
        "Pan" => {
            Unit::<RotationAngle>::parse(raw_cap, false).map(FixtureCapability::Pan)
        }
        "Tilt" => {
            Unit::<RotationAngle>::parse(raw_cap, false).map(FixtureCapability::Tilt)
        }
        s => {
...
fn parse_modes(modes: Option<&Value>) -> Option<Vec<FixtureMode>> {
    ...
    for mode in modes {
        if let Some(mode) = mode.as_object().log(|| println!("A mode must be an object")) {
            let name = mode.get("name").log(|| println!("mode must have a name")).and_then(|v| v.as_str()).map(|v| v.to_string());
            let short_name = mode.get("shortName").log(|| println!("mode must have a name")).and_then(|v| v.as_str()).map(|v| v.to_string());
            let channels = mode.get("channels").log(|| println!("mode must have channels array of strings"))
                .and_then(|c| c.as_array().map(|v| v.iter().map(|x| x.as_str()).filter(|x| x.is_some()).map(|x| x.expect("").to_string()).collect::<Vec<_>>()));
...
mod units {
    ...
    impl<T: UnitProvider> Unit<T> {
        pub fn parse(raw: &Map<String, Value>, must_be_stepped: bool) -> Option<T::R> {
            ...
            if must_be_stepped { return None; }
    ...
    impl UnitProvider for RotationAngle {
        ...
        fn assemble(start: f32, end: f32, unit: &str) -> Self::R {
            let (s, e) = match unit {
                "deg" => (PanTiltRotation::Angle(start as u32), PanTiltRotation::Angle(end as u32)),
                "%" => (PanTiltRotation::Percentage(Percentage::new(start)), PanTiltRotation::Percentage(Percentage::new(end))),
    ...
    impl UnitProvider for Brightness {
        ...
        fn assemble(start: f32, end: f32, unit: &str) -> Self::R {
            ...
            let (s, e) = match unit {
                ...
                "%" => (Br::Percentage(Percentage::new(start)), Br::Percentage(Percentage::new(end))),
                _ => unreachable!("Why here units does not match")
            };
