...
#[component]
pub fn RgbWidget(initial: (f32, f32, f32), onchange: EventHandler<(f32, f32, f32)>) -> Element {
    ...
    let hue_col = use_memo(move || {
        color_art::Color::from_hsv(color().hue(), 1.0, 1.0)
            .unwrap()
            .hex()
    });
        ...
        div { class: "rgb-widget",
            div {
                ...
                onclick: move |e| {
                    async move {
                        let s = 1.0
                            - e.element_coordinates().x
                                / sat_e().unwrap().get_client_rect().await.unwrap().size.width;
                        let v = 1.0
                            - e.element_coordinates().y
                                / sat_e().unwrap().get_client_rect().await.unwrap().size.height;
                        let old = color();
                        let v = Color::from_hsv(old.hsv_hue(), s.clamp(0.0, 1.0), v.clamp(0.0, 1.0))
                            .expect("We have a problem!");
                        color.set(v);
            ...
            div {
                ...
                onmousedown: move |e| {
                    async move {
                        let h = e.element_coordinates().y
                            / hue_e().unwrap().get_client_rect().await.unwrap().size.height * 360.0;
                        let old = color();
                        let v = Color::from_hsv(
                                h.clamp(0.0, 360.0),
                                old.hsv_saturation(),
                                old.hsv_value(),
                            )
                ...
                onkeydown: move |e| {
                    ...
                    if e.key() == Key::ArrowUp {
                        ...
                        let v = Color::from_hsv(h, old.hsv_saturation(), old.hsv_value())
                            .expect("We have a problem!");
                        color.set(v);
                        onchange.call(to_color(v));
                    }
            ...
            div {
                ...
                onclick: move |e| {
                    async move {
                        let r = e.element_coordinates().x
                            / red_e().unwrap().get_client_rect().await.unwrap().size.width * 255.0;
                        let old = color();
                        let v = Color::from_rgb(
                                r.clamp(0.0, 255.0),
                                old.green() as f64,
                                old.blue() as f64,
                            )
                ...
                onkeydown: move |e| {
                    ...
                    let old = color();
                    if e.key() == Key::ArrowLeft {
                        let r = ((old.red() as f32 / 255.0 - amount) * 255.0).clamp(0.0, 255.0)
                            as u8;
                        let v = Color::from_rgb(r, old.green(), old.blue())
                            .expect("We have a problem!");
                        color.set(v);
                        onchange.call(to_color(v));
                    }
            ...
            div {
                ...
                onclick: move |e| {
                    async move {
                        let g = e.element_coordinates().x
                            / green_e().unwrap().get_client_rect().await.unwrap().size.width * 255.0;
                        let old = color();
                        let v = Color::from_rgb(
                                old.red() as f64,
                                g.clamp(0.0, 255.0),
                ...
                onkeydown: move |e| {
                    ...
                    let old = color();
                    if e.key() == Key::ArrowLeft {
                        let g = ((old.green() as f32 / 255.0 - amount) * 255.0).clamp(0.0, 255.0)
                            as u8;
                        let v = Color::from_rgb(old.red(), g, old.blue())
                            .expect("We have a problem!");
                        color.set(v);
                        onchange.call(to_color(v));
                    }
            ...
            div {
                ...
                onclick: move |e| {
                    async move {
                        let b = e.element_coordinates().x
                            / blue_e().unwrap().get_client_rect().await.unwrap().size.width * 255.0;
                        let old = color();
                        let v = Color::from_rgb(
                                old.red() as f64,
                                old.green() as f64,
                ...
                onkeydown: move |e| {
                    ...
                    let old = color();
                    if e.key() == Key::ArrowLeft {
                        let b = ((old.blue() as f32 / 255.0 - amount) * 255.0).clamp(0.0, 255.0)
                            as u8;
                        let v = Color::from_rgb(old.red(), old.green(), b)
                            .expect("We have a problem!");
                        color.set(v);
                        onchange.call(to_color(v));
                    }
    ...
    rsx! {
        div { class: "pan-tilt-widget",
            div {
                ...
                onclick: move |e| {
                    async move {
                        let p = e.element_coordinates().y
                            / zone_e().unwrap().get_client_rect().await.unwrap().size.height;
                        let t = e.element_coordinates().x
                            / zone_e().unwrap().get_client_rect().await.unwrap().size.width;
            ...
            div {
                ...
                onclick: move |e| {
                    async move {
                        let p = e.element_coordinates().y
                            / pan_e().unwrap().get_client_rect().await.unwrap().size.height;
                        let old = pt();
            ...
            div {
                ...
                onclick: move |e| {
                    async move {
                        let t = e.element_coordinates().x
                            / tilt_e().unwrap().get_client_rect().await.unwrap().size.width;
                        let old = pt();
                ...
                async move {
                    if !e.target_touches().is_empty() {
                        let rect = size_e()
                            ...
                            .await
                            .unwrap();
                        let h = (e.target_touches()[0].client_coordinates().y - rect.origin.y)
                            / rect.size.height;
                        let v = h.clamp(0.0, 1.0) as f32;
                        val.set(v);
                        onchange.call(1.0 - v);
